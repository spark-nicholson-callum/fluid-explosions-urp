...
float2 periodicWorleyNoise(float3 P, float3 rep)
{
    float3 Pi = floor(P);
    float3 Pf = frac(P) - 0.5;
    ...
    float3 Px    = glsl_mod(Pi.x + float3(-1.0, 0.0, 1.0), rep.x);

    float3 Py_m1 = glsl_mod(Pi.yyy - 1.0, rep.y);
    float3 Py_0  = glsl_mod(Pi.yyy,       rep.y);
    float3 Py_p1 = glsl_mod(Pi.yyy + 1.0, rep.y);

    float3 Pz_m1 = glsl_mod(Pi.zzz - 1.0, rep.z);
    float3 Pz_0  = glsl_mod(Pi.zzz,       rep.z);
    float3 Pz_p1 = glsl_mod(Pi.zzz + 1.0, rep.z);

    float3 p = permute(Px);
    float3 p1 = permute(p + Py_m1);
    float3 p2 = permute(p + Py_0);
    float3 p3 = permute(p + Py_p1);

    float3 p11 = permute(p1 + Pz_m1);
    float3 p12 = permute(p1 + Pz_0);
    float3 p13 = permute(p1 + Pz_p1);

    float3 p21 = permute(p2 + Pz_m1);
    float3 p22 = permute(p2 + Pz_0);
    float3 p23 = permute(p2 + Pz_p1);

    float3 p31 = permute(p3 + Pz_m1);
    float3 p32 = permute(p3 + Pz_0);
    float3 p33 = permute(p3 + Pz_p1);
