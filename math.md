...
* $\frac{D u}{D t}$: これは速度の導関数。流体の場合は $v$ ではなく $u$ を使います。あと導関数は $d$ ではなく $D$ で書きます。これについては後ほど説明します。
* $\nu \nabla^2 u$: 粘性による力です。流体自体が動きに抵抗する力ですね。気体の場合は $\nu$ がほとんど $0$ なので、今回はこれを無視して大丈夫です。
* $\frac{1}{\rho}\nabla p$: 圧力による力です。流体の一部が周りの流体を押す力ですね。非圧縮性流体でもちゃんと存在します。
* $\frac{1}{\rho}f$: 外部からの力です。流体の場合は質量の $m$ ではなく、質量密度の $\rho$ を使いますが、ニュートンの運動方程式と同じ役割です。
...
* $\frac{\partial u}{\partial t}$: 特定の場所での速度の変化。計算したいのはこれです。
* $(u \cdot \nabla)u$: すごい書き方ですが、これは速度の移流です。流体の難しさは大体ここに詰まっています。
...
流体シミュレーションのために空間をボクセルに分けて、各場所での $\frac{\partial u}{\partial t}$ を計算します。
