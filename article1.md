...
問題は、そのまま描画すると木目のようにスライスの断面がすごく目立ってしまうため、何とかして3次元的にブレンドする必要があるということです。
...
### ジッターについて
ここまではスライスごとに描画してきましたが、スライスとスライスの間をブレンドして、ちゃんとした3次元の見た目で表示する必要があります。
これも意外と簡単で、各レイの開始位置をランダムに少しだけずらして（ジッターをかけて）計算します。
これだけだとノイズがすごくひどくなりますが、フレームごとに別の位置を選ぶようにすれば、単純なTAA（テンポラルアンチエイリアシング）をかけるだけでかなり滑らかに描画できます。

#### GPU の RNG（乱数生成）
これがよく見るGPU向けのRNG関数です。
...
とっても分かりにくい式ですよね。でも、ちゃんと `[0, 1)` の範囲の乱数になります。
大きな数字を渡すと、GPUの三角関数の精度がそこまで高くないことを利用して、結果的にほぼランダムな値が返ってくるという仕組みです。

#### ジッターの実装
本当に `rayDir` 方向にちょっとずらすだけです。
